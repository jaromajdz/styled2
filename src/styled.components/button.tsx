...
export const Button = styled.button<ButtonProps>`
  ...
  ${({ theme, color }) => {
    ...
    &:hover:not([disabled]){
        background-color:  ${
          isDark
            ? tinycolor(theme[clr]).darken(10).toString()
            : tinycolor(theme[clr]).lighten(20).toString()
        };
    ...
    &:disabled{
        background-color: ${
          isDark
            ? tinycolor(theme[clr]).darken(30).toString()
            : tinycolor(theme[clr]).lighten(30).toString()
        }; 
        color: ${
          isDark
            ? tinycolor(theme[clrc]).desaturate(50).darken(50).toString()
            : tinycolor(theme[clrc]).desaturate(50).lighten(50).toString()
        } ;
        cursor: not-allowed;
    }`;
  }}
